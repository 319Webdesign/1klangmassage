...
export function ServiceCard({ service }: { service: Service }) {
  ...
  return (
    ...
    >
      {service.isHighlight && (
        <span className="text-site-body absolute -top-3 left-6 rounded-full bg-orange-500 px-3 py-0.5 font-medium text-white">
          Tipp des Hauses
        </span>
      )}
      ...
      <div className="text-site-body mt-2 flex gap-3 text-sage-600">
        <span>{service.duration}</span>
        <span className="font-bold text-brown-600">{service.price}</span>
      </div>
      <ul className="text-site-body mt-4 space-y-2 text-brown-500/90">
